...
| **P04** | index | both | auto | Sandbox: `fts --build --limit N` completes and its documents are searchable. Prod: `fts --status` is fresh and error-free, and every un-indexed doc is EXPLAINED by an absent local body — "full" cannot mean `missing == 0` on a real Mac, where Mail keeps some folders' bodies server-side (this estate's Exchange `Sent.mbox` holds only an `Info.plist`). A doc whose `.emlx` IS on disk yet went un-indexed fails the phase; that is the only indexing failure the count can hide. |
